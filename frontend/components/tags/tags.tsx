    ...
    <div className={`group ${styles.tagContainer}`}>
      <div className="flex items-center justify-center gap-2 m-1 p-3">
        {tagsConnected.map((tag) => {
          return (
            <Badge asChild key={tag.id} className="text-sm">
        ...
        {tagsConnected.length === 0 && <span className="badge rounded-pill text-bg-light">Nog geen tags</span>}
      </div>
      {hasEditRights && (
        <div className="hidden group-hover:block group-active:block absolute right-0 top-0 translate-y-1/4">
          <Button variant="ghost" onClick={showTagsModal} icon="edit" className="m-3">
            edit tags
          </Button>
        </div>
      )}
